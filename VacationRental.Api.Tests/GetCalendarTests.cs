...
public class GetCalendarTests
{
    ...
    public async Task GivenCompleteRequest_WhenGetCalendar_ThenAGetReturnsTheCalculatedCalendar()
    {
        ...
        using (var getBooking1Response = await _client.GetAsync($"/api/v1/bookings/{postBooking1Result.Id}"))
        {
            ...
            booking1Info = await getBooking1Response.Content.ReadAsAsync<BookingViewModel>();
        }
        ...
        using (var getBooking2Response = await _client.GetAsync($"/api/v1/bookings/{postBooking2Result.Id}"))
        {
            ...
            booking2Info = await getBooking2Response.Content.ReadAsAsync<BookingViewModel>();
        }
        ...
        Assert.Contains(getCalendarResult.Dates[3].PreparationTimes, x => x.Unit == booking1Info.Unit);
        ...
        Assert.Contains(getCalendarResult.Dates[4].PreparationTimes, x => x.Unit == booking1Info.Unit);
        Assert.Contains(getCalendarResult.Dates[4].PreparationTimes, x => x.Unit == booking2Info.Unit);
        ...
        Assert.Contains(getCalendarResult.Dates[5].PreparationTimes, x => x.Unit == booking2Info.Unit);
